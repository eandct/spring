...
public class IndexController {
	...
	@Autowired
    //　Spring 管理下のオブジェクトの中から、適切なものを変数に自動でセットしてもらうため、インスタンス変数に付けるアノテーションである。
	//　自動でnewしれくれる
	...
	public String index(Model m) {
		...
		if(users != null && users.size() > 0) {
			//List.size(int index) このリスト内の指定された位置にある要素を返します。
			user = users.get(0);
            //List.get(int index) このリスト内の指定された位置にある要素を返します。
